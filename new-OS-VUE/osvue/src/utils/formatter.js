...
export function isShowColumnFormatter (array) {
  // console.log(this.$router.history.current.meta.roles)
...
export function identityType3 (type) {
  if (type === 0) return '部门月统计'
  if (type === 1) return '部门年统计'
  if (type === 2) return '相机月统计'
  if (type === 3) return '相机年统计'
  if (type === 4) return '用户月统计'
  if (type === 5) return '用户年统计'
}
export function identityType4 (type) {
  if (type === 0) return '注册人脸'
  if (type === 1) return '获取进出记录图片'
}
// 用户与账户之间的关系
...
export function trafficTypeFormatter (type) {
  if (type === 1) return '允许通行'
  if (type === 2) return '禁止通行'
}
/**
 *
 *优惠券类型
...
// LED屏显示默认传的参数
export function showTypeParams (type) {
  if (type === '0') return '项目名称'
  if (type === '1') return '进场人数'
  if (type === '2') return '离场人数'
  if (type === '3') return '在场人数'
  if (type === '4') return '欢迎xx进出工地'
  if (type === '5') return '带班人姓名'
  if (type === '6') return '班组名称'
}
...
export function happenFormatter (type) {
  if (type === 0) return '推送广告'
  if (type === 1) return '点击广告'
}
// 连接方式
...
export function timeFormatters ({ cellValue }) {
  let time = cellValue
  let result = '-'
  if (time && time !== null) {
    let date = new Date(time)
...
export function timeFormatterSortS (time) {
  let result = '-'
  if (time !== null) {
    let date = new Date(time)
    let year = date.getFullYear()
...
export function timeFormatterSort1 (time) {
  let result = '-'
  if (!time) {
    let date = new Date()
    let year = date.getFullYear()
...
// 格式化时间
export function timeHourMinSFormatter (time) {
  // console.log('格式化时间戳', time)
...
// 是否在岗/校
export function isInOutStatusFormatter (type) {
  if (type === null) return '--'
  if (type === 0) return '进场'
  if (type === 1) return '出场'
  if (type === 2) return '--'
...
export function identityFormatter (type) {
  if (type === null) return '-'
  if (type === 0) return '企业'
  if (type === 1) return '个人'
}

export function wxTypeFormatter (type) {
  if (type === null) return '-'
  if (type === 0) return '服务号'
  if (type === 1) return '小程序'
}

export function currentFormatter (type) {
  if (type === null) return '-'
  if (type === 0) return '未使用'
  if (type === 1) return '当前使用'
}

export function selfFormatter (type) {
  if (type === null) return '-'
  if (type === 0) return '本级资源'
  if (type === 1) return '父级资源'
}

export function codeTypeFormatter (type) {
  if (type === null) return '-'
  if (type === 0) return 'xml'
  if (type === 1) return 'json'
}

export function eventFeeTypeFormatter (type) {
  let temp = ''
  if (type === 'FACE_VIP') {
    temp += '人脸每台相机基本收费'
  } else if (type === 'SEND_FEE') {
    temp += '下发白名单收费'
  } else if (type === 'IMG_SAVE_FEE') {
    temp += '图片保存费用'
  } else if (type === 'RECOVERY_FEE') {
    temp += '白名单数据恢复每条费用'
  }
  temp += '(' + type + ')'
  return temp
...
export function trashFormatter (type) {
  if (type === null) return '-'
  if (type === 0) return '普通白名单'
  if (type === 1) return '高级白名单'
  if (type === 2) return '盒子白名单'
  if (type === 3) return '渣土白名单'
  if (type === 4) return '特殊车'
  if (type === 5) return '免费车'
  if (type === 6) return '盒子免费车'
  if (type === 7) return '人脸识别-用户'
  if (type === 8) return '人脸识别-账户'
}
export function trashFormatters ({ cellValue }) {
  let type = cellValue
  console.log(type)
  if (type === null) return '-'
  if (type === 0) return '普通白名单'
...
export function sceneTypeFormatter (type) {
  if (type === null) return '-'
  if (type === 4) return '广告'
  if (type === 5) return '人脸收费'
}
...
// 策略状态
export function strategyTypeFormatter (type) {
  if (type === null) return '-'
  if (type === 1) return '启用'
  if (type === 0) return '禁用'
}
...
// 转化星期
export function weekFormatter (str) {
