...
import { reflushToken } from '@/api/url.js'
// import { timeDuration } from './formatter.js'
...
export function ReToken () {
  if (flag) {
    ...
    // console.log('已登录:' + timeDuration(time / 1000))
    // 超过30分钟才重置Token
    if (time > 30 * 60 * 1000) {
      axios({
        method: 'POST',
        url: process.env.API_HOST + 'os/' + reflushToken + '/' + getTokenTime(),
        headers: {
          'AdminToken': getToken(),
        ...
        if (res.data.data.code === 1) {
          ...
          setTokenTime(res.data.data.time)
        } else {
          setTokenTime(res.data.data.time)
        }
        flag = true
