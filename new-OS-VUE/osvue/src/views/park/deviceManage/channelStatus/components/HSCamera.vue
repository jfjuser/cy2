<template>
  <div>
    <div  style="height:400px;background-size:100% 100%;display: flex;justify-content: center;align-items: center;" :style= "{backgroundImage: 'url(' + picUrl + ')'}">
      <canvas  id="line" @mousedown="clickPoint($event)" @mousemove="dragCircle($event)" @mouseup="stopDragging()">
        加载中....
      </canvas>
    </div>
...
export default {
  ...
  methods: {
    // 图像转屏幕
    IVS_I2S (shortVal, scr) {
      let val = ((shortVal) * 1000 + (1 << 13)) >> 14
      return (val * scr) / 1000
    },
    // 图像转屏幕
    IVS_S2I (val, scr) {
      let valTotal = val / scr * 1000
      let shortVal = Math.round(((valTotal << 14) - (1 << 13)) / 1000)
      return shortVal
    },
    // 加载虚拟线圈
    getVirloop (id) {
      ...
      httpGet(getVirloopById + id).then(res => {
        ...
        for (let i = 0; i < res.data.length; i++) {
          ...
          data.push({
            x: this.IVS_I2S(element.x, 760),
            y: this.IVS_I2S(element.y, 400)
          })
        }
    ...
    loopPoint (realSet, array) {
      for (let i = 0; i < array.length; i++) {
        const element = array[i]
        realSet.push({
          x: this.IVS_S2I(element.x, 760),
          y: this.IVS_S2I(element.y, 400)
        })
      }
    },
    // 保存触发区线圈
