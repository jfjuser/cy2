  ...
  <div class="app-container">
    <div class="table-main">
      ...
      <el-table stripe :header-cell-style="{background:'#f3f4f6'}" style="width: 100%" max-height="600" :data="tableData" v-loading.body="listLoading" @selection-change="handleSelectionChange" element-loading-text="加载中..." border fit highlight-current-row>
        ...
        <el-table-column label="通道名称" align="center" prop="localId"></el-table-column>
        <el-table-column label="图片" align="center" prop="imgPath"></el-table-column>
      ...
      <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
        <el-form :rules="formRules" ref="dataform" :model="form" autoCompete = "on" label-position="right" label-width="150px" >
          <el-form-item label="开闸时间" required >
            <el-date-picker
              v-model="form.date"
              type="datetime"
              placeholder="开闸时间">
          ...
          <el-form-item label="通道名称" prop="localId">
            <el-select  v-model="form.localId"  placeholder="通道名称">
              <el-option v-for="item in localIdType" :key="item.valueNo" :label="item.valueName" :value="item.valueNo"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="开闸原因" prop="reason">
              <el-input v-model="form.reason" placeholder="请输入开闸原因"></el-input>
          </el-form-item>
        </el-form>
...
export default {
  data () {
    return {
      ...
      formRules: {
        reason: [
          {required: true, trigger: 'blur', message: '开闸原因不可为空'}
        ],
        date: [
          {required: true, trigger: 'change', message: '开闸时间不可为空'}
        ],
        localId: [
          {required: true, trigger: 'change', message: '通道不可为空'}
        ]
      },
      localIdType: [],
      jsonData: [
    ...
    handleDelete () {
      this.$confirm('确认提交吗？', '提示', {})
        .then(() => {
          ...
          httpDelete(OpenRecord, {ids: this.selectArrayId}).then(res => {
