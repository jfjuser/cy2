  ...
  <div class="app-container">
    <div class="table-main">
        ...
        <div class="filter-container">
          ...
          <el-button class="filter-item vipset" type="warning" v-has="['配置部门积分']" :disabled="disabled" @click="setAllVip" icon="el-icon-set" >批量会员设置</el-button>
          <el-button class="filter-item" type="danger" v-has="['删除']"  @click="handleDelete" :loading = "deleteLoading" :disabled="disabled" icon="el-icon-close" >删除所选</el-button>
          <el-button class="filter-item vipvip" type="warning" v-has="['配置部门积分']" :disabled="disabled"  @click="priorityUpload" icon="el-icon-set" >会员优先级下发</el-button>
          <el-tag type="success" v-if="isShowColumnFormatter(['添加代理商'])" id="tag">温馨提示：代理级别部门下级可创建运营级别，也可创建代理级别</el-tag>
        </div>
        ...
        <el-table-column fixed="right" label="操作"  align="center" class-name="small-padding fixed-width" width="300" >
          <template slot-scope="scope" >
            ...
            <el-button type="primary" @click="inviteCode(scope.row)" v-if="scope.row.level === 2" size="mini">免费会员积分</el-button>
            <el-button type="primary" v-has="['测试部编辑']" style="margin-bottom:1rem" @click.native.prevent="handleOpen(scope.row)" size="mini">测试部专用</el-button>
            <el-button type="primary" v-has="['第三方配置']" @click.native.prevent="handleConfig(scope.row)" size="mini">第三方配置</el-button>
            <!-- v-has="['任务管理']"  -->
            <!-- <el-button type="primary" @click.native.prevent="eventConfig(scope.row)"  size="mini">任务管理</el-button> -->
          </template>
        </el-table-column>
        </el-table>
        <!-- 分页 -->
        ...
        <!-- 添加 -->
        <el-dialog title="任务管理" :visible.sync="dialogFormVisibles" >
          <TaskProcess v-if="dialogFormVisibles" :id="taskId"></TaskProcess>
        </el-dialog>
        ...
        <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogDetailFormVisible">
          <el-form :rules="formDetailsRules" ref="dataformDetails" :model="formDetails" autoCompete = "on" label-position="right" label-width="150px" >
              ...
              <el-form-item label="项目编号" prop="serialNumber" v-if="formDetails.themeType==9">
                  <el-input v-model="formDetails.serialNumber"  :disabled = "formDetails.themeType==9"  placeholder="请输入项目编号"></el-input>
              </el-form-item>
        ...
        <el-dialog :title="configFormTitle" :visible.sync="dialogConfigFormVisible">
          <el-form  :model="configForm" autoCompete = "on" label-position="right" label-width="150px" >
              ...
              <el-form-item label="第三方车场Id" v-if="configForm.epayType == 13">
                <el-input v-model="configForm.thirdParkId" placeholder="请输入第三方车场Id"></el-input>
              </el-form-item>
              <el-form-item label="智慧平台项目ID">
                <el-input v-model="configForm.serialNumber" placeholder="请输入智慧平台项目ID"></el-input>
              </el-form-item>
          </el-form>
          <div slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    handleDelete () {
      ...
      let message = ['删除后所选下级所有部门将不能正常使用']
      let info = {
        h,
        message,
