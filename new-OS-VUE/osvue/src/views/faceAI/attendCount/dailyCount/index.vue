  ...
  <div class="app-container">
    <div class="table-main">
      ...
      <div class="filter-container">
        <el-input class="filter-item" v-model="listQuery.userName" style="width: 100px;" :clearable="false" placeholder="姓名"></el-input>
        <!-- <el-input v-if="isShowColumnFormatter(['有为学校'])" class="filter-item" v-model="listQuery.departmentName" style="width: 100px;" :clearable="false" placeholder="班级"></el-input> -->
        ...
        <el-select clearable v-model="listQuery.type"  class="filter-item" placeholder="打卡状态">
              <el-option v-for="item in cardType" :key="item.id" :label="item.typeName" :value="item.id">
                {{ item.typeName }}
              </el-option>
          </el-select>
        <el-date-picker
      ...
      <!-- <el-table id="table" style="width: 100%;" size="mini" @cell-click="cellClick" max-height="600" :data="tableData" :row-key="rowKeys" type="index" v-loading.body="listLoading" element-loading-text="加载中..." border fit highlight-current-row>
        {{tableData}}
      </el-table> -->
      <el-table stripe id="table" style="width: 100%;" size="mini" @cell-click="cellClick" max-height="600" :data="tableData"  type="index" v-loading.body="listLoading" element-loading-text="加载中..." border fit highlight-current-row>
        ...
        <el-table-column label="上班" align="center">
          <el-table-column label="打卡时间" align="center" prop="firstBeginTime">
            <template slot-scope="scope">
              <span v-if="scope.row.firstBeginTime">{{timeHourMinFormatter(scope.row.firstBeginTime)}}</span>
               <span v-else>--</span>
          ...
          <el-table-column label="打卡结果" align="center" prop="firstBeginResult">
            <template slot-scope="scope">
               <span v-if="scope.row.firstBeginResult!==undefined">{{formatterResult(scope.row.firstBeginResult,true)}}</span>
                <span v-else>--</span>
        ...
        <el-table-column label="下班" align="center">
          <el-table-column label="打卡时间" align="center" prop="firstEndTime">
            <template slot-scope="scope">
                <span v-if="scope.row.firstEndTime">{{timeHourMinFormatter(scope.row.firstEndTime)}}</span>
                 <span v-else>--</span>
          ...
          <el-table-column label="打卡结果" align="center" prop="firstEndResult">
            <template slot-scope="scope">
              <span v-if="scope.row.firstEndResult!==undefined">{{formatterResult(scope.row.firstEndResult,false)}}</span>
              <span v-else>--</span>
              <!-- <el-button v-if="checkResult(scope.row.firstEndResult)" type="text">{{formatterResult(scope.row.firstEndResult, false)}}</el-button> -->
              <!-- <span v-else type="text">{{scope}}</span> -->
            </template>
          </el-table-column>
        </el-table-column>
        <el-table-column   label="上班2" align="center">
          <el-table-column label="打卡时间" align="center" prop="secondBeginTime">
            <template slot-scope="scope">
              <span v-if="scope.row.secondBeginTime">{{timeHourMinFormatter(scope.row.secondBeginTime)}}</span>
               <span v-else>--</span>
          ...
          <el-table-column label="打卡结果" align="center" prop="secondBeginResult">
            <template slot-scope="scope">
              <span v-if="scope.row.secondBeginResult!==undefined">{{formatterResult(scope.row.secondBeginResult,true)}}</span>
              <span v-else>--</span>
        ...
        <el-table-column  label="下班2" align="center">
          <el-table-column label="打卡时间" align="center" prop="secondEndTime">
            <template slot-scope="scope">
              <span v-if="scope.row.secondEndTime">{{timeHourMinFormatter(scope.row.secondEndTime)}}</span>
              <span v-else>--</span>
          ...
          <el-table-column label="打卡结果" align="center" prop="secondEndResult">
            <template slot-scope="scope">
               <span v-if="scope.row.secondEndResult !==undefined">{{formatterResult(scope.row.secondEndResult,false)}}</span>
                <span v-else>--</span>
        ...
        <el-table-column  label="上班3" align="center">
          <el-table-column label="打卡时间" align="center" prop="thirdBeginTime">
            <template slot-scope="scope">
              <span v-if="scope.row.thirdBeginTime">{{timeHourMinFormatter(scope.row.thirdBeginTime)}}</span>
                <span v-else>--</span>
          ...
          <el-table-column label="打卡结果" align="center" prop="thirdBeginResult">
            <template slot-scope="scope">
              <span v-if="scope.row.thirdBeginResult!==undefined">{{formatterResult(scope.row.thirdBeginResult,true)}}</span>
                <span v-else>--</span>
        ...
        <el-table-column   label="下班3" align="center">
          <el-table-column label="打卡时间" align="center" prop="thirdEndTime">
            <template slot-scope="scope">
              <span v-if="scope.row.thirdEndTime">{{timeHourMinFormatter(scope.row.thirdEndTime)}}</span>
                <span v-else>--</span>
          ...
          <el-table-column label="打卡结果" align="center" prop="thirdEndResult">
            <template slot-scope="scope">
              <span v-if="scope.row.thirdEndResult!==undefined">{{formatterResult(scope.row.thirdEndResult,false)}}</span>
                <span v-else>--</span>
        ...
        <el-table-column label="加班" align="center">
          <el-table-column label="加班时长(min)" align="center" prop="overTimeCount">
            <template slot-scope="scope">
              <span v-if="scope.row.overTimeCount">{{scope.row.overTimeCount}}</span>
                <span v-else>--</span>
              <!-- {{scope.row.overTimeCount}} -->
            </template>
          </el-table-column>
           <el-table-column label="加班申请情况" align="center" prop="remark">
            <template slot-scope="scope">
          ...
          <el-table-column label="打卡时间" align="center" prop="overTime">
            <template slot-scope="scope">
              <span v-if="scope.row.overTime">{{timeHourMinFormatter(scope.row.overTime)}}</span>
                <span v-else>--</span>
        ...
        <el-table-column label="出勤班次" align="center" prop="workShiftName"></el-table-column>
        <el-table-column label="休息天数" align="center" prop="restDay"></el-table-column>
        <el-table-column label="工作时长(分钟)" align="center" prop="workMin"></el-table-column>
      ...
      <el-dialog title="修改打卡结果" :visible.sync = "dialogAttendVisible" width="20%">
        <el-radio-group v-model="updateAttend.updateType" @change="contentChange" v-if="updateAttend.type === 1">
          ...
          <div style="margin-top: 10px;"><el-radio :label="4">请假</el-radio></div>
          <div style="margin-top: 10px;"><el-radio :label="5">出差</el-radio></div>
          <div style="margin-top: 10px;"><el-radio :label="6">申请跨天加班</el-radio></div>
        </el-radio-group>
...
import { httpGet, httpExport, httpPut, httpPost } from '@/utils/restful'
...
export default {
  data () {
    return {
      ...
      exportRules: {
        number: [{ required: true, trigger: 'change', message: '不可为空' }]
      },
      // 修改考勤
  ...
  created () {
    ...
    this.listQuery.beginTime =
      timeFormatterSort(new Date(new Date().setDate(1))) + ' 00:00:00'
    // 结束时间
  ...
  methods: {
    ...
    exportExcel () {
      ...
      // try {
      //   FileSaver.saveAs(new Blob([wbout], { type: 'application/octet-stream' }), 'daySheet.xlsx')
      // } catch (e) {
    ...
    saveUpdate () {
      ...
      httpPut(updateAIDailyCount, obj)
        .then(res => {
          this.$message({
            message: res.message,
            type: 'success',
    ...
    fetchData () {
      ...
      // console.log(1111111111111, this.listQuery)
      httpGet(AIDailyCount, this.listQuery)
        .then(res => {
          if (res.data === null) {
            self.tableData = []
            self.total = 0
            ...
            for (let item in jsonData[i]) {
              ...
              if (
                ...
                if (
                  item === 'firstBeginTime' ||
                  item === 'firstEndTime' ||
                  item === 'secondBeginTime' ||
                  item === 'secondEndTime' ||
                  item === 'thirdBeginTime' ||
                  item === 'thirdEndTime' ||
                  item === 'overTime'
                ) {
                  jsonData[i][item] = this.timeHourMinFormatter(
                    jsonData[i][item]
                  )
                }
                if (item === 'firstEndResult' ||
                   item === 'secondEndResult' ||
                   item === 'thirdEndResult'
                ) { jsonData[i][item] = this.formatterResult(jsonData[i][item], false) }
                if (
                  item === 'firstBeginResult' ||
                  item === 'secondBeginResult' ||
                  item === 'thirdBeginResult'
                ) { jsonData[i][item] = this.formatterResult(jsonData[i][item], true) }
    ...
    exportData () {
      this.$refs.exportForm.validate(valid => {
        if (valid) {
          ...
          httpExport(exportAIDailyCount, data)
            .then(res => {
    ...
    cellClick (row, column) {
      if (
        (column['property'] === 'firstBeginResult' &&
          this.checkResult(row.firstBeginResult)) ||
        (column['property'] === 'firstEndResult' &&
          this.checkResult(row.firstEndResult)) ||
        (column['property'] === 'secondBeginResult' &&
          this.checkResult(row.secondBeginResult)) ||
        (column['property'] === 'secondEndResult' &&
          this.checkResult(row.secondEndResult)) ||
        (column['property'] === 'thirdBeginResult' &&
          this.checkResult(row.thirdBeginResult)) ||
        (column['property'] === 'thirdEndResult' &&
          this.checkResult(row.thirdEndResult))
      ) {
        ...
        if (row[column.property] === -2) {
          this.updateAttend.updateType = 4
        }
        if (row[column.property] === -3) {
          this.updateAttend.updateType = 5
        }
        if (row[column.property] === 0) {
          this.updateAttend.updateType = 0
        }
        if (row[column.property] === -1) {
          this.updateAttend.updateType = 3
        }
    ...
    updateData () {
      ...
      // console.log(this.updateAttend.id)
      // 判断updateType为6时 申请跨天加班
      if (this.updateAttend.updateType === 6) {
        let obj = {
          beginTime:
            timeFormatterSort(new Date(new Date().setDate(1))) + ' 00:00:00',
          endTime: timeFormatterSort(new Date()) + ' 23:59:59',
    ...
    // 申请跨天加班
    applyOverTimes (obj) {
      httpPost(applyOverTime, obj).then(res => {
        this.dialogAttendVisible = false
        setTimeout(() => {
          this.updateLoading = false
        }, 300)
        // console.log(11111, res.message)
        if (res.message) {
          this.$message.success(res.message)
          this.fetchData()
        } else {
  ...
  directives: {
    tabs: {
      inserted (el, binding) {
        console.log(111111111111111, el, binding)
      }
    }
  }
}
