  ...
  <div>
    <div>
      ...
      <div class="filter-container">
          <el-input style="width:200px" class="filter-item" v-model="listQuery.localName" placeholder="请输入通道名称"></el-input>
        <el-input v-model = "listQuery.userName" class="filter-item" clearable style="width:200px" placeholder="姓名"></el-input>
        <!-- -----------// 有为学校 特殊设置 显示 //------------ -->
        <el-input v-has="['有为学校']" v-model = "listQuery.roomNum" class="filter-item" clearable style="width:200px" placeholder="班级"></el-input>
        <!-- -----------// 有为学校 特殊设置 不显示 //------------ -->
        <el-select clearable v-if="!isShowColumnFormatter(['有为学校'])" v-model="listQuery.aiUserId"  class="filter-item" placeholder="用户类别">
      ...
      <el-table stripe :header-cell-style="{background:'#f3f4f6'}" style="width: 100%" max-height="600" :data="tableData" v-loading.body="listLoading" element-loading-text="加载中..." border fit highlight-current-row>
        ...
        <!-- -----------// 有为学校 特殊设置 显示 //------------ -->
        <el-table-column label="学校" v-if="isShowColumnFormatter(['有为学校'])" align="center" prop="school"></el-table-column>
        <el-table-column label="班级" v-if="isShowColumnFormatter(['有为学校'])" align="center" prop="roomNum"></el-table-column>
...
import { getAIPic, exportInout, checkAILinuxCamera, syncAIInOut, getAITreeInOut } from '@/api/url'
export default {
  data () {
    return {
      time: [],
  ...
  methods: {
    ...
    fetchData () {
      ...
      httpGet(getAITreeInOut, this.listQuery).then(res => {
        if (res.data === null) {
          self.tableData = []
          self.total = 0
        } else {
    ...
    handleImage (row) {
      ...
      httpPut(getAIPic + row.id + '/' + row.inOutTime).then(res => {
