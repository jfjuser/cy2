  ...
  <div class="app-container">
    <div class="table-main">
      ...
      <div class="filter-container">
        ...
        <el-button class="filter-item controll" type="primary" v-has="['实时控制']" @click="controlOnTime" :disabled="disabled" icon="el-icon-date">实时控制</el-button>
        <el-button class="filter-item set" type="primary" @click="setParams" :disabled="disabled" icon="el-icon-date">参数设置</el-button>
        <el-button class="filter-item getfile" type="primary" v-has="['获取文件']" @click="getFiles" :disabled="disabled" icon="el-icon-date">获取文件</el-button>
      </div>
...
import { cloudAICamera, getAISnap, deleteAICameraFile, downAIFile, getAndroidAIFiles, getLinuxAIFiles, getFilesList, AIcameraParamsList, updateCamera, AIsentCommandList, getOrderList, getAIFile, UpgradeAIHttpCamera, unBindAICamera } from '@/api/url'
...
export default {
  ...
  created () {
    this.fetchData()
    // console.log('查看数据', this.onprogress)
  },
  methods: {
    handleh () {
      ...
      httpGet(cloudAICamera, this.listQuery).then(res => {
        if (res.data === null) {
          ...
          res.data.rows.map((item) => {
            // console.log(item)
            if (item.cameraType === 134 && item.tcpVersion === '2020-03-18_114130') {
