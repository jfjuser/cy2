        ...
        <el-tabs type="border-card">
          <el-tab-pane v-for="(item, index) in list" :key="index" :label="item[0].classify">
            <el-form-item v-for="(obj, index) in item" :key="index" :label="obj.chineseName" required>
              ...
              <!-- {{obj}} -->
               <el-radio-group v-model="obj.value[0]" v-if="obj.value instanceof Array">
                  <el-radio v-for="(option, index) in obj.options" :key="index" :label="option.value">{{option.label}}</el-radio>
                </el-radio-group>
                <el-radio-group v-model="obj.value" v-else>
                  <el-radio v-for="(option, index) in obj.options" :key="index" :label="option.value">{{option.label}}</el-radio>
                </el-radio-group>
              </div>
              <!-- 多选框 -->
      ...
      this.$confirm('是否对相机的参数进行保存？', '提示', {})
        .then(() => {
          ...
          this.array.forEach((item, index) => {
            if (item.type === 5 || item.selectType === 2 || item.name === 'serverIP' || item.name === 'serverPort' || item.name === 'user_id' || item.name === 'user_passwd') {
              console.log(22222)
              if (item.name === 'serverIP' || item.name === 'serverPort' || item.name === 'user_id' || item.name === 'user_passwd') {
                // return false
              } else {
                let data = 0
                ...
                // console.log(item.fileName, item.name, data)
                arr.push({
              ...
              // console.log(item.fileName, item.name, item.value)
              if (item.value) {
                arr.push({
                  fileName: item.fileName,
                  key: item.name,
