  ...
  methods: {
    ...
    confirm () {
      ...
      if (!(normalPlate.test(this.plate) || newPlate.test(this.plate))) {
        ...
        axios({
          method: "POST",
          url: this.host + "pv/v1/yunPay/queryFee",
          data: data
        })
          .then(res => {
            if (res.status === 200 && res.data.code === 1) {
              if (res.data.data.authCode === '') {
                res.data.data.authCode = this.authCode
              }
              let params = encodeURIComponent(JSON.stringify(Object.assign(data, res.data.data)))
