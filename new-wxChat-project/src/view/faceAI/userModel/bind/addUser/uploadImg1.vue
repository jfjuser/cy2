<template>
  <div class="uploadImg">
    <!-- 上传图片 -->
    <!-- <img :src="imgs" style="width:60px;height:60px;" /> -->
...
import { faceAICheckPhoto } from '@/api/url'
    ...
    beforeUploadRead (file) {
      ...
      if (file) {
        ...
        if (!isJPG) {
          ...
          console.log(file)
          console.log(9999)
          this.compress(file, 4).then(res => {
            return this.successUploadImg(this.dataURLtoBlob(res), file.name)
          })
      ...
      await lrz(file, {
        ...
        exifimg(file).then((res1) => {
          ...
          lrz(res1, {
            ...
            return _this.successUploadImg(res2.file, res.origin.name)
          })
        })
      })
    },
        ...
        reader.onload = e => {
          ...
          if (num !== 1) {
            img.onload = function () {
              ...
              cvs.width = img.width / num
              cvs.height = img.height / num
              ctx.drawImage(img, 0, 0, img.width, img.height, 0, 0, cvs.width, cvs.height)
              resolve(cvs.toDataURL('image/jpeg'), 0)
            }
          } else {
            resolve(imgData)
          }
