<template>
  <div class="warp-content">
    ...
    <div class="ast" style="margin-top:1.5rem">
      <Field  disabled required v-model="beginTime" @click="showTime" input-align="right" right-icon="arrow" label="出差开始时间" placeholder="请选择时间" />
    </div>
...
<!-- 出差结束时间 -->
    <div class="ast" style="margin-top:1.5rem">
      <Field  disabled required v-model="endTime" @click="showEndTime" input-align="right" right-icon="arrow" label="出差结束时间" placeholder="请选择时间" />
    </div>
    ...
    <!-- 出差地点 -->
    <van-field  style="margin-top:1rem" v-model="businessTrip" label="出差地点" placeholder="请输入出差地点" />
<!-- 出差事由 -->
...
<div v-for="(item, index) in reviewerList" :key="item.userName" class="approved">
<van-tag closeable plain type="primary" size="large"  @close="deleteReviewer(index)">{{item.userName}}</van-tag>
