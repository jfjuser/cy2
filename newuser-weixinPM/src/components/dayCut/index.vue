<template>
  <div>
    ...
    <div class="date">
      ...
      <span @click="getNow" style="font-size:15px">回到今天</span>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  watch: {
    ...
    nowDate (newDate, oldDate) {
      if (new Date(newDate).valueOf() > new Date().valueOf()) {
        this.$notify({
          type: 'warning',
          message: `请选择历史日期查看某天考勤情况！`,
          duration: 3000
        })
      } else {
      }
      this.$emit('timeDot', newDate)
    }
  },
  data () {
    return {
      ...
      var mySwiper = new Swiper('.swiper-container', {
        on: {
          ...
          slidePrevTransitionEnd: () => {
            ...
            console.log(this.currentFirstDate + 'cjs')
