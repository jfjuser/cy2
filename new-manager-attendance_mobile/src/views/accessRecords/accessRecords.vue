...
<script>
import { Search, Button, PullRefresh, List, Cell, Tag, Empty, Calendar, Popup } from 'vant'
...
export default {
  ...
  methods: {
    ...
    exportData () {
      this.$http
        .get(aiInOutRecords, { params: this.listQuery })
        .then(res => {
    ...
    viewPicture (item) {
      ...
      this.$http
        .put(inOutPhoto + item.id + '/' + item.inOutTime)
        .then(res => {
    ...
    getData () {
      let list = Object.assign(this.listQuery, {departmentId: this.$store.getters.departmentId, dcCode: this.$store.getters.departmentId})
      // let list = Object.assign(this.listQuery, {departmentId: 7235, dcCode: 7235})
      let self = this
      // 线上
      this.$http
        .get(page, { params: list })
        .then(res => {
          let data = res.data
          if (data.rows !== null) {
