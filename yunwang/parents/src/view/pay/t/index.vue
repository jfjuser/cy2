    ...
    <div style="padding-top:15px">
      <div id="div_ticket" style="padding:10px;border-radius:4px;background:white;font-size: 15px;">
        <p>&#x3000;&#x3000;恭喜您获得商家<span class="info">{{busyInfo.shopName}}</span>优惠券一张，
          ...
          <span v-if="busyInfo.subType === 3">停车打<span class="info">{{busyInfo.item}}</span>折
          ，立即绑定车牌使用吧!</span></p>
...
<script>
import { Ticketbingding, TicketbingdingInfo } from '@/api/url.js'
...
export default {
  ...
  created () {
    this.$vux.toast.text('加载中，请稍后....', 'bottom')
  },
  mounted () {
    this.getUserInfo()
  },
  methods: {
    ...
    getUserInfo () {
      ...
      if (href.indexOf('?') > -1) {
        ...
        this.$vux.toast.text('连接地址获取失败!', 'bottom')
      }
    },
    // 获取车场信息
    getInfo () {
      ...
      httpGet(TicketbingdingInfo, params).then(res => {
        // console.log('车辆接口查看22', res)
    ...
    // 提交验证车牌
    confirm () {
      const normalPlate = /^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领A-Z]{1}[A-Z0]{1}[A-HJ-NP-Z0-9]{4}[A-HJ-NP-Z0-9挂学警港澳]{1}$/
      const newPlate = /^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领A-Z]{1}[A-Z0]{1}(([0-9]{5}[DF]$)|([DF][A-HJ-NP-Z0-9][0-9]{4}$))/
      if (!(normalPlate.test(this.plate) || newPlate.test(this.plate))) {
        ...
        httpPost(Ticketbingding, params).then(res => {
          // console.log('车辆接口查看11', res)

          this.$router.push({path: '/t/tip/'})
        }).catch(() => {
        })
      }
    }
