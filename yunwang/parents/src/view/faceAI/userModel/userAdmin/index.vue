<template>
  <div class="user-admin">
    ...
    <group class="list-content" @change="selectEvent" v-if="!isBind">
    ...
    <group class="list-content" @change="selectEvent" v-else>
      <!-- inline-desc="详情说明"  value="value" link='#' :is-link='true' -->
      <cell
        v-for="(item, index) in userAdminList1"
        :title="item.title"
        :value="item.value"
        :key="index"
        @click.native.prevent="selectEvent(item.id)"
...
import { loginByPhone, getAiuserMessage } from '@/api/url'
...
export default {
  ...
  data () {
    return {
      ...
      userAdminList1: [
        { id: 2, icon: iconPhone, title: '用户信息绑定', value: '已绑定' }
      ],
      isBindPhone: false,
      isBind: false,
      loginForm: {
