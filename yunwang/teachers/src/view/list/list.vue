<template>
  <div class="warp-content">
    ...
    <div v-if="tabActive === 0">
      <van-dropdown-menu>
    <van-dropdown-item @change="searchType" v-model="formData.attendanceType" :options="option2" :disabled="ismm" />
...
>
<div>
  <!-- @click="toDetailsPage(item.vacationId)" -->
<div  class="card" v-for="item in applyList" :key="item.vacationId" style="padding: 0.5rem 0;">
  <div class="card-content">
    ...
    <div v-if="tabActive === 1">
      <van-dropdown-menu>
            <van-dropdown-item @change="approveSearchType" v-model="approvalFormData.attendanceType" :options="option2" :disabled="ismm"  />
...
<!-- <van-tabbar v-model="tabActive"> -->
  ...
  <!-- v-if="ifApprovalAuthority" -->
 <!-- <van-tabbar-item  icon="bookmark">我审批的</van-tabbar-item>
</van-tabbar>
...
import { getPowerByParams, listVacation, approveVacation, getAiuserMessage, doApproveVacationByStudent } from '@/api/url'
// import { mapGetters } from 'vuex' // 引入vuex辅助函数
import { timeFormatter } from '@/utils/formatter'
import { getDepInfo, getUserInfo, getFaceInfo } from '@/utils/userInfo.js'
...
export default {
  ...
  methods: {
    ...
    // 确认审核结果
    toConfirmResults () {
      httpPost(doApproveVacationByStudent,
    ...
    // 获取申请的条目
    getSubmitted () {
      this.formData.aiUserId = getUserInfo().id
    ...
    // 获取审批的条目
    getApproval () {
      this.approvalFormData.approveUserId = getUserInfo().id
