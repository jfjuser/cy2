<template>
  <div class="warp-content">
    ...
    <div v-if="tabActive === 1">
      <van-dropdown-menu>
    <van-dropdown-item @change="searchType" v-model="formData.attendanceType" :options="option2" />
...
import { getPowerByParams, listVacation, approveVacation } from '@/api/url'
...
export default {
  ...
  methods: {
    ...
    // 确认审核结果
    toConfirmResults () {
      this.$http.post(approveVacation,
    ...
    getApprovalAuthority () {
      let that = this
      this.$http.get(getPowerByParams + `/${this.departmentId}/${this.userId}`).then(res => {
    ...
    // 获取申请的条目
    getSubmitted () {
      this.formData.aiUserId = this.userId
      this.formData.departmentId = this.departmentId
      this.$http.post(listVacation, this.formData).then(res => {
